...
import com.tynkovski.notes.presentation.pages.authorization.state.SignUpInputState
...
class SignUpViewModel(
...
) : ViewModel(), ContainerHost<SignUpViewModel.State, SignUpViewModel.SideEffect> {
    ...
    fun signUp() = intent {
        authRepository
            ...
            .onSuccess {
                ...
                postSideEffect(SideEffect.UserRegister(token = it.token))
            }
            .onError {
    ...
    data class State(val buttonState: ButtonState)

    sealed interface SideEffect {
        data class UserRegister(val token: String) : SideEffect
    }
}